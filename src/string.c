...
void string_add( CHAR_DATA *ch, char *argument )
{
    ...
    if ( *argument == '/' && str_cmp(argument, "/s"))
    {
	...
	strcpy( tmparg3, argument );
	    ...
	    *ch->desc->pString = str_dup( "" );
        ...
        if ( !str_cmp( arg1, "/f" ) || !str_cmp(arg1, "/fi"))
        {
	...
	send_to_char( "/d <num>         - delete line number <num>\n\r", ch);
	send_to_char( "/i <num> <str>   - insert <str> on line <num>\n\r", ch);
	send_to_char( "/e <num> <str>   - replace line <num> with <str>\n\r", ch);
    ...
    if ( !str_cmp(argument, "/s") || *argument == '@' )
    {
